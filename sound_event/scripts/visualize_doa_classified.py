...
# Distinct colors for different class labels
CLASS_COLORS = {
    "human": '#1f77b4',      # blue
    "tank": '#ff7f0e',       # orange
    "vehicle": '#2ca02c',    # green
    "aircraft": '#d62728',   # red
...
def get_class_color(class_label: str) -> str:
    """Get color for a class label."""
    return CLASS_COLORS.get(class_label.lower(), FALLBACK_COLORS[hash(class_label) % len(FALLBACK_COLORS)])
...
class ClassifiedDOAVisualizer:
    ...
    def process_classified_audio(self, audio: np.ndarray, class_label: str):
        """
        Process a classified audio segment.
        
        Parameters
        ----------
        audio : np.ndarray
            Audio array of shape (4, n_samples) - 4-channel audio
        class_label : str
            Class label for this audio segment (e.g., "human", "tank")
        """
        # Validate input
        if audio.ndim != 2 or audio.shape[0] != 4:
            raise ValueError(f"Audio must be shape (4, n_samples), got {audio.shape}")
        
        # Add to queue (non-blocking, drop if queue is full)
        try:
            self.audio_queue.put_nowait((audio, class_label))
        except queue.Full:
    ...
    def _processing_loop(self):
        ...
        while not self.stop_processing.is_set():
            try:
                ...
                audio, class_label = self.audio_queue.get(timeout=0.1)
                ...
                if self.sos is not None:
                    audio = apply_filter(self.sos, audio, mode="zero_phase")
                # # Process the entire segment
                # results = self.pipeline.process_block(audio)
                for i in range(0, audio.shape[1], self.audio_cfg["block_size"]):
                    results = self.pipeline.process_block(audio[:,i:i+self.audio_cfg["block_size"]])
                    ...
                    if results:
                        ...
                        for track in tracks:
                            ...
                            track_dict["class_label"] = class_label
                ...
                logger.error("Error processing audio segment", extra={"error": str(e)})
    ...
    def get_latest_snapshot(self) -> Optional[Dict]:
        ...
        with self._snapshot_lock:
            ...
            return {
                ...
                "tracks": list(self._latest_snapshot["tracks"]),
                "timestamp_sec": self._latest_snapshot["timestamp_sec"],
                "class_label": self._latest_snapshot["class_label"],
            }
    ...
    def start(self):
        ...
        def update(_):
            ...
            for track in track_data:
                ...
                colors.append(get_class_color(track["class_label"]))
...
def main():
    ...
    # Start in separate thread so we can push audio
    import threading
    ui_thread = threading.Thread(target=visualizer.start, daemon=True)
    ui_thread.start()
    ...
    print("\n=== Test Mode ===")
    print("This is a demo. In real usage, call visualizer.process_classified_audio() from your project.")
    print("Press Ctrl+C to stop.\n")
    
    try:
        # Example: simulate receiving audio segments
        # In real usage, you would call visualizer.process_classified_audio() from your project
        while True:
            time.sleep(5.0)
            print("(In real usage, call visualizer.process_classified_audio(audio, class_label) from your project)")
    except KeyboardInterrupt:
        print("\nStopping...")
    finally:
        visualizer.stop()

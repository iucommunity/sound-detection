...
from src.my_doa.pipeline.distance_estimation import (
    estimate_distance_for_class_window,
    CLASS_PARAMS,
...
class ClassifiedTrack:
    ...
    # Smoothing history
    theta_history: deque = field(
        default_factory=lambda: deque(maxlen=UI_SMOOTHING_FRAMES)
    )
...
def _merge_close_tracks(track_data: list, merge_threshold_deg: float = 40.0) -> list:
    ...
    for class_label, class_tracks in by_class.items():
        ...
        sorted_tracks = sorted(
            class_tracks, key=lambda t: t["confidence"], reverse=True
        )
        ...
        for i, track in enumerate(sorted_tracks):
                    ...
                    for close_track in close_tracks:
                        dist = abs(
                            circular_distance_deg(
                                close_track["theta_deg"], other_track["theta_deg"]
                            )
                        )
            ...
            if len(close_tracks) == 1:
                ...
                distances = np.array([t["distance_m"] for t in close_tracks])
                spls = np.array([t["spl_db"] for t in close_tracks])
...
class ClassifiedDOAVisualizer:
    ...
    def __init__(self):
        ...
        self.pipe_cfg, self.audio_cfg = load_pipeline_config(
            "sound_event/config/pipeline.yaml"
        )
        ...
        print(
            f"Expected audio shape: (4, n_samples) where n_samples = {int(1.5 * self.fs)} for 1.5s"
        )
            ...
            try:
                ...
                for sep_audio in sep_audios:
                    ...
                    if self.sos is not None:
                        sep_audio["audio"] = apply_filter(
                            self.sos, sep_audio["audio"], mode="zero_phase"
                        )

                pipelines = [DOAPipeline(self.pipe_cfg) for _ in range(len(sep_audios))]
                audio_samples = sep_audios[0]["audio"].shape[1]

                for i in range(0, audio_samples, self.audio_cfg["block_size"]):
                    snapshots = []
                    for index, sep_audio in enumerate(sep_audios):
                        x_c_block = sep_audio["audio"][
                            :, i : i + self.audio_cfg["block_size"]
                        ]
                        results = pipelines[index].process_block(x_c_block)
                        ...
                        if results:
                            ...
                            # ---New: Distance estimation for this class + block
                            L_db = None
                            r_hat = None

                            if tracks:
                                doas_deg = [tr.theta_deg for tr in tracks]
                                try:
                                    L_db, r_hat = estimate_distance_for_class_window(
                                        x_c_chunk=x_c_block,
                                        fs=self.fs,
                                        class_id=sep_audio["class_name"],
                                        doas_deg=doas_deg,
                                        class_params=CLASS_PARAMS,
                                        calibration_offset_db=CALIBRATION_OFFSET_DB,
                                    )
                                except Exception as e:
                                    # Don't crash visualization if parameters missing, just log
                                    logger.error(
                                        f"Distance estimation failed for class = {sep_audio['class_name']}:{e}"
                                    )
                            ...
                            for track in tracks:
                                ...
                                # Attach distance information if successfully computed
                                if r_hat is not None:
                                    track_dict["distance_m"] = r_hat
                                if L_db is not None:
                                    track_dict["spl_db"] = L_db

                                classified_tracks.append(track_dict)

                            # Update snapshot
                            snapshot = {
                                "frame_index": pipelines[index].frame_index,
                ...
                logger.error(
                    f"Error processing audio segment {e}", extra={"error": str(e)}
                )
    ...
    def start(self):
        ...
        frontend_address = "192.168.130.227"
        frontend_port = 9999

        def update():
            nonlocal last_snapshot_frame
            ...
            for snapshot in snapshots:
                ...
                for ui_track in self.ui_tracks.values():
                    ...
                    if len(ui_track.theta_history) > 0:
                        smoothed_theta = _compute_circular_mean(
                            list(ui_track.theta_history)
                        )
                    else:
                    ...
                    if len(ui_track.confidence_history) > 1:
                        ...
                        smoothed_conf = (
                            0.6 * ui_track.confidence_history[-1] + 0.4 * prev_mean
                        )
                    ...
                    if (
                        np.isnan(smoothed_theta)
                        or np.isnan(smoothed_conf)
                        or np.isinf(smoothed_theta)
                        or np.isinf(smoothed_conf)
                    ):
                        continue
                ...
                if len(track_data) > 1:
                    ...
                    track_data = _merge_close_tracks(
                        track_data, merge_threshold_deg=40.0
                    )
                ...
                for track in track_data:
                    ...
                    if (
                        np.isnan(theta_deg)
                        or np.isnan(confidence)
                        or np.isinf(theta_deg)
                ...
                tracked_datas.append(track_data)

            sock.sendto(json.dumps(tracked_datas).encode(), (frontend_address, frontend_port))

        interval_sec = int(1 / UI_FPS)
        try:
            while True:
                update()
